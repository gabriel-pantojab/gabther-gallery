...
export function CreateAlbumModal({ parentId, close }: Props): JSX.Element {
	const { formData, changeForm, updateForm, handleCreateAlbum, onDrop } =
		useCreateAlbum({
			parentId,
			close,
		});
		...
		<div className='fixed left-0 top-0 z-50 flex h-full w-full items-center justify-center bg-gray-400 bg-opacity-50 px-4'>
			...
			>
				...
				<div className='flex w-full flex-col gap-2'>
					...
					>
						<input {...getInputProps()} />
						{formData.cover === null && !isDragActive && (
